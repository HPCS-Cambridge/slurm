/***************************************************************************** \
 *  task_cgroupv2_blkio.c - blkio cgroup subsystem for task/cgroup
 *****************************************************************************
...
extern int task_cgroupv2_blkio_init(slurm_cgroup_conf_t *slurm_cgroup_conf, xcgroup_t *slurm_cgroupv2_root)
{
...
static int blkio_cgroupv2_initialize (xcgroup_ns_t *ns, xcgroup_t *cg, char *path, uid_t uid,
...
//	char *slurm_cgpath;
//
//	/* create slurm root cg in this cg namespace -> done */
//	slurm_cgpath = task_cgroupv2_create_slurm_cg(&root_ns, &slurm_cg);
...
//	 */
//	if (xcgroup_create(&root_ns, &blkio_cg, "",0,0) != XCGROUP_SUCCESS) {
...
//	 */
//	if (xcgroup_create(&root_ns, &user_blkio_cg,
//			    user_cgroup_path,
...
//	 */
//	if (blkio_cgroupv2_initialize (&root_ns, &job_blkio_cg, job_cgroup_path,
//	                      getuid(), getgid(), 0) < 0) {
//		xcgroup_destroy (&user_blkio_cg);
//		goto error;
//	}
...
//	 * manually removed by the plugin at the end of the step.
//	 */
//	if (blkio_cgroupv2_initialize (&root_ns, &step_blkio_cg, jobstep_cgroup_path,
//	                      uid, gid, 0) < 0) {
//		xcgroup_destroy(&user_blkio_cg);
//		xcgroup_destroy(&job_blkio_cg);
//		goto error;
...
extern int task_cgroupv2_blkio_prepare_step(stepd_step_rec_t *job, xcgroup_t *cg)
{
	//int fstatus = SLURM_ERROR;
	...
	xcgroup_set_param(cg, "io.weight", io_qos);
